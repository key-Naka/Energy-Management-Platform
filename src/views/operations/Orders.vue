<template>
    <el-card>
        <el-row :gutter="20">
            <el-col :span="6">
                <el-input aria-placeholder="" placeholder="请输入订单号" v-model="searchParams.orderNo">
                </el-input>
            </el-col>
            <el-col :span="6">
            ...
            <el-col :span="6" class="mt">
                <el-date-picker type="daterange" range-separator="/" start-placeholder="开始日期" end-placeholder="结束日期"
                    v-model="date" @change="dateChange" value-format="YYYY-MM-DD">
                </el-date-picker>
            </el-col>
        </el-row>
    </el-card>
...
const selectionList = ref<selectionType[]>([])
const handleSelectChange = (val: selectionType[]) => {
    selectionList.value = val
}
const handleBatchDelete = async () => {
    try {
        const res = await batchDeleteApi(selectionList.value.map((item: selectionType) => item.orderNo))
        if (res.code == 200) {
...
const handleDetail = (orderNo: string) => {
    ...
    setcurrentTap("订单详情", `/operations/detail`)
