...
import type { MenuType } from "@/type/user";
...
function collectUrls(tree: MenuType[]) {
    const urls: string[] = [];
    function traverse(node: MenuType) {
        if (node.url && !node.children) {
            urls.push(node.url)
        }
        if (node.children) {
            node.children.forEach((child: MenuType) => traverse(child))
        }
    }
    tree.forEach((node: MenuType) => traverse(node));
    return urls
}
...
const settingAuth = async (pageAuthority: string, account: string) => {
    ...
    console.log(pageAuthority,account);
